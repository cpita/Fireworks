var Tochamen = function (x,y,colorR,colorG,colorB) {
    this.x = x;
    this.y = y;
    ...
    this.update = function () {
        for(var i = 0;i < BASIC_FIREWORK_SIZE;i++){
            ...
            ctx.fill();
            for(var j = 1;j <= 10;j++){
                ctx.fillStyle = "rgba("+fireworks[i].colorR+","+fireworks[i].colorG+","+fireworks[i].colorB+","+fireworks[i].colorA/j+")";
                ctx.beginPath();
                ctx.arc(fireworks[i].x + fireworks[i].initX - j * fireworks[i].dX,fireworks[i].y+fireworks[i].initY+j*parabola(fireworks[i].f,fireworks[i].x,fireworks[i].dX > 0),fireworks[i].radius,0,2*Math.PI);
                ctx.closePath();
                ctx.fill();
            }
        }
